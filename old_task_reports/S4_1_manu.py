...
def daily_report_S4_1(df, save_path, date):
    ...
    # Assicurati che 'STATE_wrong_side_START' sia di tipo float
    error_df['STATE_wrong_side_START'] = error_df['STATE_wrong_side_START'].astype(float)

    print("antes del primer for")

    # Condizione per param != 0.00000
    condition = error_df['STATE_wrong_side_START'] != 0.00000

    # Imposta error a 1 dove la condizione è soddisfatta
    error_df.loc[condition, 'error'] = 1

    # Imposta error_R a 1 dove la condizione è soddisfatta e side è 'right'
    error_df.loc[condition & (error_df['side'] == 'right'), 'error_R'] = 1

    # Imposta error_L a 1 dove la condizione è soddisfatta e side è 'left'
    error_df.loc[condition & (error_df['side'] == 'left'), 'error_L'] = 1
