...
def daily_report_S1_manu(df, save_path, date):
    ...
    # 1 PLOT: latency to the first correct poke
    axes = plt.subplot2grid((1600, 50), (1, 1), rowspan=350, colspan=90)
    plt.plot(df.trial, df.response_latency, color='dodgerblue')
    ...
    axes = plt.subplot2grid((1600, 50), (500, 1), rowspan=110, colspan=90)
    labels = df.correct_outcome_int.values.copy()
    labels[df['first_trial_response'] == 'no_response'] = 2
    # Combina i dati da entrambe le colonne nel grafico a dispersione
    sns.scatterplot(x=df.trial, y=df['first_trial_response'], hue=labels,
                    palette=[incorrect_c, correct_c, miss], s=30, ax=axes)

    axes.set_title(summary)

    plt.xlabel('First response')
    plt.ylabel('Trial')
    ...
    # axes = plt.subplot2grid((1600, 50), (700, 1), rowspan=110, colspan=90)
    ...
    # axes = plt.subplot2grid((1600, 50), (900, 1), rowspan=110, colspan=90)
    # # missed_trials_df = df[df['missed_reward'] == 1]

    # sns.scatterplot(x=df['trial'], y=df['missed_reward'], s=30, ax=axes,
    #                 color=['darkorange' if x == 1 else 'royalblue' for x in df['missed_reward']])
    # axes.set_yticks([0, 1])
    ...
    axes = plt.subplot2grid((1600, 50), (1200, 1), rowspan=150, colspan=90)

    df['rolling_accuracy'] = compute_window(df["correct_outcome_int"], 5) * 100

    plt.plot(df.trial, df.rolling_accuracy, color='blueviolet', linestyle='-', linewidth=1, marker='o')
