...
def daily_report_S4(df, save_path, date):
    ...
    formatted_session_duration = "{:.2f}".format(
        df['session_duration'].iloc[0])
    df['center_light_duration'] = df['STATE_center_light_END'] - \
        df['STATE_center_light_START']
    df['side_light_duration'] = df['STATE_side_light_END'] - \
        df['STATE_side_light_START']
    df['duration_drink_delay'] = df['STATE_drink_delay_END'] - \
        df['STATE_drink_delay_START']
    ...
    # median latency to first response
    df['center_response_latency_median'] = df['centre_response_latency'].median()

    # median latency to first response
    df['side_response_latency_median'] = df['side_response_latency'].median()

    # misses: no reward from the sides but they can poke anyway
    # max time in side_light (for extrac missed trial)
    max_duration_side_light = 300
    df['missed_reward'] = (df['side_light_duration'] ==
                           max_duration_side_light).astype(int)
    count_missed_trials = df['missed_reward'].sum()
    # missed_reward_rate = (df['missed_reward'].iloc[0] /
    #                       df['trials_max'].iloc[0]) * 100
    ...
    df['missed_center_poke'] = (
        df['center_light_duration'] == max_duration_centre_light).astype(int)
    ...
    df['total_poke_left'] = exploded_port2_df.groupby(
        exploded_port2_df.trial)['left_poke'].sum()
    df['total_poke_centre'] = exploded_port3_df.groupby(
        exploded_port3_df.trial)['central_poke'].sum()
    df['total_poke_right'] = exploded_port5_df.groupby(
        exploded_port5_df.trial)['right_poke'].sum()
    ...
    poke_df['total_poke_before_correct'] = poke_df.poke_before_correct_left + \
        poke_df.poke_before_correct_centre + poke_df.poke_before_correct_right
    poke_df['total_poke_each_trial'] = poke_df.total_poke_left + \
        poke_df.total_poke_centre + poke_df.total_poke_right
    poke_df['total_poke'] = poke_df['total_poke_each_trial'].sum()

    # microliters each poke, but i want the result in milliliters
    df['water_intake'] = (df['tot_correct_choices'].iloc[0]) * 0.01
    ...
    columns = df[["trial", "side",
                  "STATE_wrong_side_START", 'correct_outcome_int']]
    ...
    error_df['error_rate'] = (
        error_df['error_count'].iloc[0]/error_df['trial'].iloc[-1]) * 100
    ...
    column_names = ['trial', 'side', 'correct_outcome_int',
                    'first_response_left', 'first_response_center', 'first_response_right']
    ...
    for i, row in first_lick_df.iterrows():
        ...
        if row["first_resp_right"] == 1:
            ax.plot(trial, y_coord, marker='|',
                    color='purple', markersize=markersize)
    ...
    for block in df['Block_index'].unique():
        ...
        # Calcola il punto centrale del blocco
        block_center = (start + end) / 2
        # Probabilità del blocco
        ...
        ax.hlines(y=1.5, xmin=start, xmax=end, colors=color,
                  linestyles='solid', linewidth=10)
    ...
    ax.text(1.02, 0.9, 'R', ha='left', va='bottom',
            color='purple', transform=ax.transAxes, fontsize=10)

    # Posiziona la lettera 'C' appena fuori dal bordo destro del grafico
    ax.text(1.02, 0.45557, 'C', ha='left', va='bottom',
    ...
    plt.plot(df.trial, df.centre_response_latency,
             color='black', label='Centre')

    # Personalizzazione dei ticks sull'asse y

    # I valori che desideri mostrare sull'asse y
    custom_yticks = [0, 1, 10, 20, 50, 100, 200, 250, 300]
    axes.set_yscale('log')
    axes.set_yscale('log')
    ...
    # calculate missing and omission per port
    column_names = ['trial', 'side', 'correct_outcome_int',
                    'first_response_left', 'first_response_center',
    ...
    for i, row in first_lick_df.iterrows():
        ...
        # Per 'left'
        if row["first_resp_left"] == 1:
            axes.plot(trial, y_coord, marker='|',
                      color='green', markersize=markersize)
    ...
    for i in range(num_blocks):
        ...
        center = (start + end) / 2
        axes.text(center, 1.6, formatted_mean_prob, ha='center',
                  va='center', backgroundcolor='white', fontsize=5)
    ...
    if len(df) % blocks_lenght != 0:
        ...
        formatted_mean_prob = f"{mean_prob:.2f}"
        center = (start + end) / 2
        axes.text(center, 1.6, formatted_mean_prob,
    ...
    for i in range(num_blocks):
        ...
        # Disegna una linea orizzontale per il blocco
        axes.hlines(y=1.5, xmin=start, xmax=end, colors=color,
                    linestyles='solid', linewidth=10)
    ...
    for i in range(num_blocks + 1):  # +1 per includere la linea finale del grafico
        ...
        axes.vlines(x=x_pos, ymin=-0.2, ymax=1.2, colors='grey',
