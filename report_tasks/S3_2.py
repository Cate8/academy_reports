...
def daily_report_S3_2(df, save_path, date):
    ...
    session_summary = f'''
     ...
     (R: {poke_df['total_n_poke_right'].iloc[0]}, C: {poke_df['total_n_poke_centre'].iloc[0]}, L: {poke_df['total_n_poke_left'].iloc[0]}), Errors: {error_df['error_count'].iloc[0]} (R: {error_df['error_R_count'].iloc[0]}, L: {error_df['error_L_count'].iloc[0]}), Error rate: {error_df['error_rate'].iloc[0]}'''
    ...
    for i, row in first_lick_df.iterrows():
        ...
        y_coord = 1.4 if markersize == 5 else 1.2
    ...
    for i, row in first_lick_df.iterrows():
        ...
        y_coord = -0.4 if markersize == 5 else -0.2
    ...
    # ERRORS: Water_delivery_state == 0
    for trial, param in enumerate(df['STATE_water_delivery_START']):
        if param == 0:
            if df.side.iloc[trial] == "right":
                plt.scatter(trial, -0.05, color='red', marker="v", s=15)
            elif df.side.iloc[trial] == "left":  # Make sure to use consistent case ('Left' vs 'left')
                plt.scatter(trial, 1.05, color='red', marker="^", s=15)
