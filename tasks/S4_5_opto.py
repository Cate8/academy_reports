...
def daily_report_S4_5(path, save_path):
    ...
    count_opto_trials = ((df['opto_bool']) & (df['iti_duration'] > 6)).sum()
    df['opto_trials'] = (df['opto_bool'] & (df['iti_duration'] > 6)).sum()
    ...
    condition = (df['opto_bool'] == 1) & (df['iti_duration'] > 6)
    ...
    # PSICOMETRIC: Second plot using subplot2grid
    #
    ...
    # ax2.plot(x, probit(x, *pars), label='Probit Fit')
    # ax2.scatter(probs, right_choice_freq, marker='o')
    # ax2.set_ylim(0, 1)
...
root_folder = "C:\\academy_reports\\academy_reports\\sessions\\raton1"
...
for root, dirs, files in os.walk(root_folder):
    for file in files:
        if file.endswith(".csv"):
            ...
            #if not "raw" in file and "S4" in file and not os.path.exists(save_path): # requires manually delete the generated pdf in order to generate a new one
            if not "raw" in file and "S4_5_" in file: # overwrite the pdf to the previous one

                daily_report_S4_5(path, save_path)
